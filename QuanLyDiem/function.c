...
bool contains_space(const char *str) {
    while (*str) {
        if (*str == ' ') {
            return true; // Nếu tìm thấy dấu cách, trả về true
        }
        str++;
    }
    return false; // Nếu không tìm thấy dấu cách, trả về false
}
void menu()
{
...
void read_input_file(SinhVien *list,  int *quality, FILE *file)
{
    ...
    int count = 0, index = 0;
    char* token = NULL;
    while((fgets(buffer, sizeof(buffer), file)) != NULL && count <= MAX_SV)
    {
...
void add_student(SinhVien *list, int *quality) {
    if (*quality == MAX_SV) {
        ...
        if (value.name[strlen(value.name) - 1] == '\n') {
            value.name[strlen(value.name) - 1] = '\0';
        }

        while (strlen(value.name) == 0 || contains_space(value.name) == 0) {
            printf("\nVui long nhap lai ten sinh vien (nhap phai co dau cach va khong duoc rong): ");
            fgets(value.name, sizeof(value.name), stdin);
            if (value.name[strlen(value.name) - 1] == '\n') {
                value.name[strlen(value.name) - 1] = '\0';
            }
...
void edit_student(SinhVien *list, int quality, char *id_student)
{
    ...
    else
    {
        ...
        bool flag;
        SinhVien *temp = search(list, quality, &flag, id_student);
        if(flag)
        {
            char answer = '\0';
            char sel = 0;
